...
export async function getSignedInRole() {
  const api = client();
  ...
  const { data, error } = await api
    ...
    .single();
  if (error) throw error;
  return data.role as "admin" | "jury" | "participant";
}
...
export async function getJuryDashboard() {
  ...
  const { data: userData, error: userError } = await api.auth.getUser();
  if (userError || !userData.user) throw new Error("Please sign in again.");
  const [juryResult, projectsResult, criteriaResult, rankingResult] =
    await Promise.all([
      api
        .from("jury_members")
        .select("id, full_name")
      ...
      api
        .from("projects")
        .select(
          "id, project_name, category, description, problem_statement, solution, github_url, demo_url, pitch_deck_url",
        )
        .order("created_at"),
  ...
  if (error) throw error;
  return {
    jury: juryResult.data as Jury,
    projects: projectsResult.data as Project[],
    criteria: criteriaResult.data as Criterion[],
    ranking: rankingResult.data as Ranking[],
  };
}
...
export async function uploadPitchDeck(projectId: string, file: File) {
  ...
  if (file.size > 10 * 1024 * 1024)
    throw new Error("Pitch deck must be 10 MB or smaller.");
  const api = client();
  const { data: userData, error: userError } = await api.auth.getUser();
  if (userError || !userData.user) throw new Error("Please sign in again.");
